...
```mermaid
flowchart TD
   c((LoRa Recieve interupt))-->d[[LoRa data receive handler]]

   e(CORE 0)-->f[[LoRaTask \nhandle all LoRa processing]]
   e-->g[[System Support \nWiFi, Bluetooth,FreeRTOS scheduler]]
...
2. If the Receiver has not been given an IP address from the Server, a screen will appear on the Receiver display. The last line will show the IP address the Receiver is using for it's local AP IP address. Record this address.
3. On a PC that is already connected to the WiFi network, change the WiFi connection to the Receiver temporary WiFi SSID **"WaterGaugeReceiver"** with the password:

   ```
   password123
   ```

4. In a browser window on a PC, enter this local AP IP address and a WiFi Manager screen will appear.

5. Press **Configure WiFi**.
6. If a list of networks is displayed:
   - Select the appropriate network.
   - Enter the SSID and password.
   - Save the settings.
7. Once the Receiver accepts the new network credentials, it will obtain an IP address from the selected network.
8. The assigned IP address will be displayed on the Receiver display.
