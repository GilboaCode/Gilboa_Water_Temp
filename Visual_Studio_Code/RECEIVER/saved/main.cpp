...
#define OLED_ON_PB 19 // Short to ground to activate the OLED display on the Receiver for 30 seconds
int OLED_On_Timer_Counter = 0; // Timer to track how long the OLED display has been on, in seconds
bool OLED_On_Flag = false; // Flag to indicate if the OLED display is on or off
bool displayOnCommand = false; // Flag to indicate if the display has been commanded to turn on
...
void loadRecipients() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    Serial.printf("Loaded recipient email[%d]: %s\n", i, recipient_emails[i].c_str());
  }
  prefs.end();
  String tempS = "Loaded ";
  tempS  += String(6);
...
void loadSenderCreds() {
  ...
  Serial.print("Loaded sender email: "); Serial.println(sender_email);
  sender_password = prefs.getString("password", "efocbnzvxhcfwngx");
  Serial.print("Loaded sender password: "); Serial.println(sender_password);
  prefs.end();
...
void updateOLED() {
  ...
  if (OLED_On_Flag == true) {
    ...
    u8g2.sendBuffer();
    digitalWrite(Display_On_Pin, LOW); //  Turn on display
  }
  else {
    u8g2.clearBuffer();
    u8g2.sendBuffer();
    digitalWrite(Display_On_Pin, HIGH); //  Turn off display
  }
}
...
void command_superhelp (String chat_id, String text){
  ...
  runningText += " /rom       - ROM assignments";
  runningText += "\n";
  runningText += " /disp_on   - Receiver display on";
  runningText += "\n";
  runningText += " /debug_on  - Sender Debug mode on";
  runningText += "\n";
  runningText += " /debug_off - Sender Debug mode off";
  runningText += "\n";
  runningText += " /WDT_on    - Watchdog mode on";
  runningText += "\n";
  runningText += " /WDT_off   - Watchdog mode off";
  runningText += "\n";
...
// Command display-on, Turn on the display of the Receiver for 30 seconds
void command_display_on (String chat_id, String text) {
  displayOnCommand = true;
  bot.sendMessage(chat_id, "Display turned ON for 30 seconds", "");
}
...
void handleNewMessages(int numNewMessages)
{
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if (text == "/disp_on") command_display_on (chat_id,text);
    if (text == "/debug_on") command_debug_on (chat_id,text);
    if (text == "/debug_off") command_debug_off (chat_id,text);
    if (text == "/WDT_on") command_WDT_on (chat_id,text);
...
// Turn on the OLED of the Receiver
void turnOnOLED() {
  OLED_On_Flag = true; // Turn on OLED display
  digitalWrite(Display_On_Pin, LOW); // Power on the OLED display
  OLED_On_Timer_Counter = millis(); // Reset the OLED timer countdown
  updateOLED();
}


// Check the OLED on Input pin and turn on the OLED display if pressed
void checkOLEDOnPin() {
  if (OLED_On_Flag == false) {
    if (digitalRead(OLED_ON_PB) == LOW) {
      turnOnOLED();
      Serial.println("OLED turned ON via button press");
    }
    if (displayOnCommand) {
      turnOnOLED();
      displayOnCommand = false; // Clear the command flag
      Serial.println("OLED turned ON via Telegram command");
    }
  }
}
...
// ============================================================
// USB NVS BACKUP
// Sends all selected NVS variables to the PC over USB Serial
// ============================================================

void exportNVSBackup() {

  // Make sure all current values are loaded from NVS first
  loadGraphLimits();
  loadEmailConfig();
  loadRecipients();
  loadSenderCreds();
  ...
  // GRAPH
  // ----------------------------------------------------------
  Serial.print("graph.minY=");
  Serial.println(graphMinY, 6);

  Serial.print("graph.maxY=");
  Serial.println(graphMaxY, 6);

  // ----------------------------------------------------------
  // EMAIL
  // ----------------------------------------------------------
  Serial.print("email.recipient_email=");
  ...
  // ----------------------------------------------------------
  for (int i = 0; i < 6; i++) {
    Serial.print("recipients.email");
    Serial.print(i);
    Serial.print("=");
    Serial.println(recipient_emails[i]);
  }
  ...
  Serial.println(sender_email);

  Serial.print("sender.password=");
  ...
  // OPERATOR
  // ----------------------------------------------------------
  Serial.print("operator.aux_sleep=");
  Serial.println(aux_sleep_minutes);

  Serial.print("operator.oled_flag=");
  Serial.println(OLED_flag_from_receiver ? 1 : 0);
...
xTaskCreatePinnedToCore(
  ...
  pinMode(OLED_ON_PB, INPUT_PULLUP); // Pin to trigger hard reset of WiFi settings
  ...
  digitalWrite(Display_On_Pin, LOW); // Power on the OLED display
  OLED_On_Timer_Counter = millis(); // Start the OLED timer countdown
...
void loop() {
  ...
  checkOLEDOnPin(); // Check OLED pin for turning on the OLED display
...
//-------------------------
  ...
  // Handle the OLED timer countdown
  if (OLED_On_Flag == true) {
    if (millis() - OLED_On_Timer_Counter > 30000) {
      OLED_On_Flag = false;
      updateOLED();
    }
  }
